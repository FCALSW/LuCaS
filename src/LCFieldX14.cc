...
LCFieldX14::LCFieldX14(G4String evfile)
{
  ...
  G4cout<< "LCFieldX14:: Initializing DID Field map from: "<< fileName << G4endl; 
    ...
    G4Exception("LCFieldX14:: cannot open file: ",
		evfile,
		RunMustBeAborted,
		" run aborted !");
  }
  ...
  G4cout << " found " << Nz << " points in the map " << G4endl;
    for ( iz=0 ; iz < Nz ; iz++ ){
      ...
      //  G4cout<< Bz[iz] / tesla << " " << Br[iz] / tesla << "  " << Bx[iz] / tesla << G4endl;
    }
    inputFile.close();
  zStep = zPos[1] - zPos[0];
  ...
  G4cout<< "LCFieldX14:: DID Field map initialized in range: "
	<< -zPos[Nz-1]/mm 
	<< " < Z < " 
	<< zPos[Nz-1] /mm << " [mm] " << G4endl
	<< G4endl; 
}
    ...
    if(std::abs(Point[2])<zmax && (sqr(Point[0])+sqr(Point[1]))<rmax_sq)
{
  ...
  const G4int signZ = (Point[2] >= 0) ? (+1) : (-1); // (ignore z = 0 here)
